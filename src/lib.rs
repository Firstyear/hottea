...
use ocl::{enums::DeviceInfo, enums::DeviceInfoResult, flags, Device, Platform, ProQue};
...
impl HotTea {
    ...
    #[inline(always)]
    pub(crate) fn do_sd(&mut self, d: &[f64], x: f64) -> f64 {
        self.do_sd_cpu(d, x)
    }

    #[inline(always)]
    pub(crate) fn do_mean(&mut self, d: &[f64]) -> f64 {
        self.do_mean_cpu(d)
    }
}
...
impl HotTea {
    pub fn new() -> Self {
        // Need a much better way to select this in the future.
        let plat = Platform::default();
        let devices = Device::list(plat, Some(flags::DeviceType::new().system_default())).unwrap();
        let first = devices.get(0).unwrap();
        ...
        let wg_size = if let Ok(DeviceInfoResult::MaxWorkGroupSize(v)) =
            pro_que.device().info(DeviceInfo::MaxWorkGroupSize)
        {
            v
        } else {
            panic!("Failed to get opencl wg size");
        };
        ...
        info!("OCL device wgsize -> {:?}", wg_size);
    ...
    pub(crate) fn do_mean_ocl(&mut self, d: &[f64]) -> f64 {
        ...
        // self.pro_que.set_dims(self.wg_size);
        ...
        input_buffer.write(d).enq().expect("Failed to write data");
        ...
        unsafe {
            kernel1.cmd().enq().expect("failed to queue kernel");
        }
...
mod tests {
    ...
    fn hottea_t_test_basic() {
        ...
        let t_r = t.t_test(&a, &b);
        // account for slight f64 differences between ocl && cpu.
        assert!(t_r == -8.213501426846603 || t_r == -8.213501426846609);
    }
    ...
    fn hottea_sd_basic() {
        ...
        #[cfg(feature = "ocl_support")]
        {
            ...
            let data: [u32; 3] = [1024 << 3, 1024 << 7, 1024 << 14];
            // let data: [u32; 1] = [1 << 14];
            for l in &data {
                let l = *l;
                println!("========= {}", l);
